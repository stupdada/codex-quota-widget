...
- 7 天窗口会优先使用同一时间段 5 小时消耗换算出的“小数 7 天消耗”来计算近速需留；官方 7 天整数消耗只作为诊断和比例学习依据，避免 `92% -> 91%` 这类整数跳变被直接外推到最右侧。
- 5 小时到 7 天的换算比例以 `0.17` 作为初始值，并随本地历史里的同段 5 小时/7 天消耗缓慢学习，当前约束范围为 `0.12` 到 `0.25`。
